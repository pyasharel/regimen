...
export const StreakCard = () => {
  ...
  if (isLoading) {
    return (
      <Card className={`p-3 border ${
        isRefinedMode 
          ? 'bg-[hsl(var(--streak-fire)/0.08)] border-[hsl(var(--streak-fire)/0.2)]' 
  ...
  const fireColor = isRefinedMode ? 'text-[hsl(var(--streak-fire))]' : 'text-orange-500';
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/50">
          <div className="flex items-center gap-1.5">
            <Trophy className={`w-3.5 h-3.5 ${fireColor}`} />
            <div>
          ...
          <div className="flex items-center gap-1.5">
            <Target className={`w-3.5 h-3.5 ${fireColor}`} />
            <div>
